...
sim_l = simulations_pref %>% 
  group_by(l) %>%
  summarise(Cav_av = mean(Cavi), Cav_se = sd(Cavi)/sqrt(max(simulations_pref$n_sim)),
            Fav_av = mean(Favi), Fav_se = sd(Favi)/sqrt(max(simulations_pref$n_sim)))
...
C_F_pref = 
  ...
  scale_x_continuous(breaks = c(0.1, 0.9),
                     labels = c('0.1', '0.9'))+
  scale_y_continuous(breaks = c(0.1, 1.1),
                     labels = c('0.1', '1.1'))+
...
coal_res_pref = read.csv('../data/coalescence_clean_results_0.0new_cost_original.csv')
#DATA WRANGLING
...
for (i in seq(n_sim)){
  ...
  x = coal_res_pref_i$Cav1- coal_res_pref_i$Cav2 + coal_res_pref_i$Fav2  - coal_res_pref_i$Fav1
  #Normalize x
  x = x/max(abs(x))
...
similarity_micro = 
  ...
  scale_x_continuous(expand = c(0,0),
                     limits = c(-0.46, 0.47),
                     breaks = c(-0.45, 0, 0.45),
                     labels = c("-0.45",  "0", "0.45"))+
