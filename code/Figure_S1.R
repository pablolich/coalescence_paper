...
coal_rep = read.csv('../data/effect_facilitation_oldy_cost.csv', row.names = 1 )
...
grouped = 
  coal_rep %>% group_by(la, lb) %>% summarize(S_av = mean(S), S_sd = sd(S))

old_cost =
  ggplot(data = grouped,
       aes(x = lb, y = S_av))+
  geom_ribbon(aes(x = lb, ymin = S_av - S_sd, ymax = S_av + S_sd,
                  fill = as.factor(la)), alpha = 0.3)+
  geom_line(size = 2, aes(group = la,
                          colour = as.factor(la)))+
  ...
  scale_color_manual(values = c("#D7191C", "darkolivegreen4", '#2C7BB6'), name = '', 
                                labels = c('$l_A = 0.1$', '$l_A = 0.5$', '$l_A = 0.9$'))+
  scale_fill_manual(values = c("#FC8D59", "darkolivegreen3", '#91BFDB'), name = '', 
                    labels = c('$l_A = 0.1$', '$l_A = 0.5$', '$l_A = 0.9$'))+
  scale_x_continuous(expand = c(0, 0),
                     breaks = c(0.2, 0.5, 0.8))+
  labs(x = "Leakage of community $B$ $(l_B)$",
       y = "Similarity to parents $(S_{A, B})$")

#Load data regarding feasibility

feas_old = read.csv('../data/feasible_space_old_cost.csv', row.names = 1)

#Merge and average
data = feas_old %>% left_join(feas_new, by = "l_val") %>% group_by(l_val) %>% summarise(R_av_old = mean(R.x), R_sd_old = sd(R.x))
feasibility = ggplot(data = data) + 
   geom_ribbon(aes(x = l_val, ymin = R_av_old - R_sd_old, 
                   ymax = R_av_old + R_sd_old), alpha = 0.3)+
  # geom_ribbon(aes(x = l_val, ymin = R_av_new - R_sd_new, 
  #                 ymax = R_av_new + R_sd_new), alpha = 0.3)+
  geom_line(size = 2, aes(x = l_val, y = R_av_old))+
  # geom_line(size = 2, aes(x = l_val, y = R_av_new))+
  ...
  labs(y = "Average resouce concentration $(R^{\\star})$",
       x = "Leakage $(l)$")

#Plot alltogether
lay <- rbind(c(1, 1, 2, 2, 2))
tikz("../sandbox/figure_S1.tex",
     width = 8, height = 8.5,
     standAlone = TRUE)
grid.arrange(feasibility, old_cost,
             layout_matrix = lay)
dev.off()
